...
def create_user_route(user: UserCreate, session: Annotated[Session, Depends(get_session)]) -> UserRead:
...
def read_user_route(current_user: Annotated[User, Depends(get_current_user)], session: Annotated[Session, Depends(get_session)]) -> UserRead:
    if not current_user:
...
def update_user_route(current_user: Annotated[User, Depends(get_current_user)], user: UserCreate, session: Annotated[Session, Depends(get_session)]) -> UserRead:
...
def delete_user_route(current_user: Annotated[User, Depends(get_current_user)], session: Annotated[Session, Depends(get_session)]) -> dict:
